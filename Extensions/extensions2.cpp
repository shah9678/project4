...
vector<Mat> target_images = {
    imread("/Users/aditshah/Desktop/PRCV/Project4/Extensions/test.jpeg", IMREAD_GRAYSCALE),
    imread("/Users/aditshah/Desktop/PRCV/Project4/Extensions/test1.jpeg", IMREAD_GRAYSCALE)
};

void detectAndPlaceARObject(Mat& frame) {
    for (const auto& target_image : target_images) {
        if (target_image.empty()) {
            cerr << "Error: One of the target images not found!" << endl;
            continue;
        }
        ...
        if (good_matches.size() > 10) {
            ...
            if (!H.empty()) {
                ...
                // Extension 2: Adding creative virtual objects and scenes
                circle(frame, frame_corners[0], 20, Scalar(0, 0, 255), -1); // Red dot at the first corner
                rectangle(frame, frame_corners[2] - Point2f(20, 20), frame_corners[2] + Point2f(20, 20), Scalar(255, 255, 0), -1); // Cyan square at the third corner
                putText(frame, "Virtual Scene", frame_corners[1], FONT_HERSHEY_SIMPLEX, 0.7, Scalar(0, 255, 255), 2);
            }
        }
    }
}
